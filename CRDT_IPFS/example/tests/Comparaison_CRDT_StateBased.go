...
func BootstrapStateBasedSetUp(cfg Config.CRONUSConfig) {
	...
	go SendState(SetCrdt1, cfg.NtpServ, file, sys1.Cr.Id, sema, cfg)
...
func Peer_NotUpdating(cfg Config.CRONUSConfig) {
...
func Peer_Updating(cfg Config.CRONUSConfig) {
	sema := semaphore.NewWeighted(1)
...
func doUpdates(nbUpdates int, SetCrdt1 *CLSet.CRDTCLSetStateBasedDag, ntpServ string, file *os.File, netID string, sema *semaphore.Weighted) {
	ti := time.Now()
	k := 0
	for k < nbUpdates {
